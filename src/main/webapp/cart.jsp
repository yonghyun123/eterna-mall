...
<body>
											...
											<c:forEach var="cart" items="${list }" varStatus="status">
												<tr>
													...
													<td class="cart-button text-center"><input
														type="checkbox" value="${cart.productId }"  name="deleteCheckbox">
                                                    <input type="text" value="${cart.productId }" class="hiddenDelete"></td>
													<!-- 상품 정보 -->
					...
					<div class="col-md-6">
						<div class="row mb-5">
							<div class="col-md-6 mb-3 mb-md-0">
								<button class="btn btn-primary btn-sm" id="DeleteCheckBtn" type="button">선택 삭제</button>
							</div>
						</div>
					</div>
					</form>
	...
	<script type="text/javascript">
		...
		$(document).on("click", "#DeleteCheckBtn", function(event) {
			var productIdList = document.getElementsByName('deleteCheckbox');
			var productId = {};
...
// 	      		form.submit();
		...
		$(document).on("click", ".btn-outline-primary", function(event) {
			...
			for ( var key in count) {
				var countList = count[key].value;

			}
// 		      for (var i = 0; i < count.length; i++) {
// 		    	  if(count[i].checked ){
// 		    		productId=count[i].value;
// 		    		productId = {
// 		    				productId : productId
// 		    		}
// 		    	  }
